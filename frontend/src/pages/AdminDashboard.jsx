...
const { user, authenticated, ready } = usePrivy();
  ...
  return (
    <div className="flex h-screen bg-zinc-50 dark:bg-black text-zinc-900 dark:text-zinc-100">
      {/* SIDEBAR */}
      <aside className={`${isSidebarOpen ? 'w-64' : 'w-20'} bg-white dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-800 transition-all duration-300 flex flex-col`}>
        <div className="p-6 flex items-center gap-3">
          <div className="w-8 h-8 bg-[#F26722] rounded-lg flex-shrink-0" />
          {isSidebarOpen && <span className="font-black italic tracking-tighter text-xl">NERO <span className="text-[#F26722]">ADMIN</span></span>}
        </div>
      ...
      <main className="flex-1 overflow-y-auto p-8">
        <header className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-zinc-500 text-sm">Panel de control de Mercado Nero</p>
          </div>
        ...
        <section className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          {activeTab !== 'orders' && (
            <div className="h-64 flex items-center justify-center border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-[2.5rem]">
              <p className="text-zinc-400 font-medium italic">Sección en desarrollo...</p>
            </div>
          )}
        </section>
      </main>

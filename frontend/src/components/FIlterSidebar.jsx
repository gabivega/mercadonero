 ...
 const handlePriceRange = (min, max) => {
  ...
  const clearAllFilters = () => {
    onFilterChange({
      minPrice: '',
      maxPrice: '',
      sort: filters.sort, // Mantenemos el orden
      brand: '',
      condition: '',
      freeShipping: false,
      subCategory: ''
  ...
  return (
    <aside className="w-full md:w-72 flex-shrink-0 self-start space-y-3 pb-10">
      
      {/* Cabecera: Título y Limpiar */}
        ...
        <div className="flex flex-col">
          {['new', 'used'].map((cond) => (
            <button 
              ...
              className={`text-xs text-left px-3 rounded-xl transition-all ${
      ...
      {/* Precio */}
  <section>
        <h3 className="text-xs font-black uppercase mb-4 dark:text-white tracking-widest">Precio</h3>
          ...
          {[
            ...
            <li key={idx}>
              <button 
                ...
                className="group flex items-center justify-between w-full text-[11px] text-zinc-500 hover:text-black dark:hover:text-white transition-colors"
              >
                {range.label}
      ...
      {/* Marcas */}
      <section>
        <h3 className="text-xs font-black uppercase mb-4 dark:text-white tracking-widest">Marcas</h3>
          ...
          {availableBrands.map((brand) => (
            <button 
              ...
              className={`w-full text-left px-3 py-2 rounded-xl text-xs transition-all ${
                filters.brand === brand 
                ? 'bg-[#F26722] text-white font-bold' 
                : 'text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800'
              }`}
            >
              {brand}
      ...
      <section>
        <h3 className="text-xs font-black uppercase mb-4 dark:text-white tracking-widest">Subcategorías</h3>
        <div className="space-y-1 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
          {availableSubCategories.map((subCategory) => (
            <button 
              key={subCategory}
              onClick={() => onFilterChange({ ...filters, subCategory: subCategory === filters.subCategory ? '' : subCategory })}
              className={`w-full text-left px-3 py-2 rounded-xl text-xs transition-all capitalize ${
...
function Badge({ label, onRemove }) {
  return (
    <span className="flex items-center gap-1.5 px-3 py-1.5 bg-zinc-100 dark:bg-zinc-800 text-[10px] font-black uppercase rounded-full border border-zinc-200 dark:border-zinc-700 dark:text-zinc-300 shadow-sm">
      {label}
